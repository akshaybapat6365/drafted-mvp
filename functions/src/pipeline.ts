...
import { generatePlanGraph, renderPlanSvg } from "./geometry";
import {
  ProviderHttpError,
  generateExteriorImage,
  generateHouseSpec,
} from "./gemini";
import { logEvent } from "./logging";
import { bucket, db } from "./runtime";
import type { ArtifactDoc, HouseSpec, JobDoc, ProviderCallMeta } from "./types";
...
export async function processQueuedJob(jobId: string): Promise<void> {
  ...
  if (!claimed) return;
  logEvent("functions_worker", "job_claimed", {
    job_id: jobId,
  ...
  for (let attempt = claimed.retryCount ?? 0; attempt <= maxRetries; attempt += 1) {
    ...
    try {
      ...
      logEvent("functions_worker", "job_succeeded", {
        job_id: jobId,
        retry_count: attempt,
      });
      return;
      ...
      if (classification.retryable && attempt < maxRetries) {
        logEvent("functions_worker", "job_retry_scheduled", {
          job_id: jobId,
      ...
      logEvent("functions_worker", "job_failed", {
        job_id: jobId,
        retry_count: attempt,
        failure_code: classification.code,
        error: safeMessage(err),
...
async function claim(jobRef: DocumentReference): Promise<JobDoc | null> {
...
async function runPipeline(
  ...
  const calls: ProviderCallMeta[] = Array.isArray(job.providerMeta?.calls)
    ? [...job.providerMeta.calls]
    : [];

  let spec: HouseSpec;
  if (job.providerMeta?.reuseSpec && job.parentJobId) {
    const parentSnap = await db.collection("jobs").doc(job.parentJobId).get();
    const parent = parentSnap.data() as JobDoc | undefined;
    if (!parent?.houseSpec) throw new Error("validation:reuse_spec_unavailable");
    ...
    const specResult = await generateHouseSpec({
      prompt: job.prompt,
  ...
  const planGraph = generatePlanGraph(spec);
  ...
  const svgBuffer = Buffer.from(renderPlanSvg(planGraph), "utf-8");
  ...
  if (job.wantExteriorImage) {
    ...
    const image = await generateExteriorImage({
      prompt: job.prompt,
      style: job.style,
    });
    if (image) {
...
function validateSpec(job: JobDoc, spec: HouseSpec): void {
  const beds = spec.rooms.filter((r) => r.type === "bedroom").length;
  const baths = spec.rooms.filter((r) => r.type === "bathroom").length;
  if (beds < job.bedrooms) throw new Error("validation:bedrooms_mismatch");
  if (baths < job.bathrooms) throw new Error("validation:bathrooms_mismatch");
...
function classifyFailure(err: unknown): { code: string; retryable: boolean } {
  if (err instanceof ProviderHttpError) {
    if (TRANSIENT_HTTP_CODES.has(err.status)) return { code: "provider_transient", retryable: true };
